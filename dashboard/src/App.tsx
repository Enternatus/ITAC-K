  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--base)', display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <header style={{
        ...
        padding: '0 20px', paddingRight: 224, borderBottom: '1px solid var(--border)', background: 'var(--header-bg)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {/* Simulation/Mock Data Badge */}
          {isMock && (
            <Tooltip text="Running synthetic simulation stream (MockProvider). ESP32 physical edge hardware disconnected.">
              <div style={{
      ...
      {/* Main content — 200px right margin for the persistent theme panel */}
      <main style={{ flex: 1, padding: '20px 24px', marginRight: 200, ...dimStyle }}>
        {screen === 'overview' && (
          <Overview
            slots={slots}
            peers={peers}
      ...
      <ThemePicker activeId={themeId} onSelect={setThemeId} />
    </div>
  );
}
...
export default function App() {
  return (
    <ITACDataProvider initialMode="mock">
      <DashboardContent />
    </ITACDataProvider>
  );
}
